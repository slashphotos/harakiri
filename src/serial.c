...
static void evaluateOtherData(uint8_t sr)
{
    ...
    case '#':
        ...
        break;
    case 'M':
        cfg.tele_protocol_init = 1;      // activate mavlink
        break;
    case 'R':
        if (! f.ARMED) {
...
void serialCom(void)
{
    ...
    while (uartAvailable())
    {
        ...
        if (cfg.tele_protocol_init == 1 && ! cliMode)
        {
            if (c == '\r' && last_c[1] == '\r' && last_c[0] == '\r')
            {
                cfg.tele_protocol_init = 0;
                cliProcess();
            }
            last_c[1] = last_c[0];
            last_c[0] = c;
            baseflight_mavlink_receive(c);
            continue;
        }
