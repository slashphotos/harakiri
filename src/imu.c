...
int16_t         BaroP;            // Crashpilot
int16_t         BaroI;            // Crashpilot
int16_t         BaroD;            // Crashpilot
uint8_t         newbaroalt;       // Crashpilot
uint32_t        BaroFstTime;      // Crashpilot
static int32_t  GroundAlt;        // Crashpilot
static uint16_t ACCDeltaTime;     // Crashpilot
uint16_t        BaroDeltaTime;    // Crashpilot
float           actual_speed[2];
float           ACCDeltaTimeINS = 0;
static float    VelUP = 0;        // Crashpilot
...
// This is the core of the INS stuff
    ...
    if (sensors(SENSOR_MAG) && cfg.mag_calibrated == 1){
        ...
        tmp0 = atan2f(-Yh,Xh) * 180.0f / M_PI;                         // Get rad to Degree
			  flthead = tmp0 + magneticDeclination;                          // Add Declination
        if (flthead > 180.0f) flthead = flthead - 360.0f;              // Wrap to -180 0 +180 Degree
         else if (flthead < -180.0f) flthead = flthead + 360.0f;
		} else flthead = 0;                                                // if no mag do bodyframe below
    heading   = flthead;                                               // Heading in Degrees
 		flthead   = flthead * RADX;                                        // Do GPS INS rotate ACC X/Y to earthframe no centrifugal comp. yet
		cy        = cosf(flthead);
    sy        = sinf(flthead);
...
void getEstimatedAltitude()
{
  ...
  if (currentTime < BaroFstTime) GroundAlt = BaroAlt;
	if (newbaroalt!=0){                                          // MS Baro Timecheck 27ms // BMP085 Timecheck 26ms debug[0] = BaroDeltaTime/1000;
      ...
      tmp32 = 0;
		  for (IdxCnt = 0; IdxCnt < BaroTabsize; IdxCnt++) tmp32 = tmp32 + BaroTab[IdxCnt];
      ...
      LastEstAltBaro = EstAltBaro;
		  fltmp = 0;
		  for (IdxCnt = 0; IdxCnt < VarioTabsize; IdxCnt++) fltmp = fltmp + VarioTab[IdxCnt];
