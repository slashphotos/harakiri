...
void annexCode(void)                                 // Crashpilot: execute in computeIMU
{
    ...
    if (feature(FEATURE_LED)) {
    	if (cfg.LED_Type == 1) {	//MWCRGB
    		...
    		else if (buzzerFreq > 4) LED_Value = 2200;
    		else {
    ...
    if (gyro.temperature) gyro.temperature(&telemTemperature1);
    else {
        // TODO MCU temp
    }
} // END OF annexCode 
...
// Don't enable Killswitch when Notarmed or Rc Signal disturbed
    ...
    if (!f.ARMED || rcOptions[BOXARM] == 1 || cfg.killswitchtime == 0 || failsafeCnt > 1) Killtimer = 0;
            ...
            AltRCTimer0 = currentTime + 100000;
                ...
                if (LastAltThrottle > cfg.minthrottle && thrdiff < 0){
                    Althightchange = -1;
							  ...
							  if (ThrFstTimeCenter == 1 && Althightchange != 0) AltHold8 = (EstAlt + ((float)vario * cfg.baro_lag)) * 8;    // We are coming from a hightchange project stoppingpoint
							  Althightchange = 0;
						}
        }                                                                                                                     // End of X Hz Loop
        AltHold = AltHold8 >> 3;
...
/*
                ...
                initialThrottleHold = initialThrottleHold + (BaroP / 100);							  // Adjust Baselinethr by 1% of BaroP							
							  if (LastAltThrottle < cfg.maxthrottle && thrdiff >= 0)
						        AltHold8 = AltHold8 + thrdiff - cfg.alt_hold_throttle_neutral;
                if (LastAltThrottle > cfg.minthrottle && thrdiff < 0)
                    AltHold8 = (float)AltHold8 + ((float)thrdiff * 0.6f) + cfg.alt_hold_throttle_neutral;                     // Descent only with half speed
            } 
*/

        if (sensors(SENSOR_GPS)){
            GPS_alltime();                                                            // Do INS GPS stuff here
            if (!DoingGPS() || !f.GPS_FIX_HOME) GPS_reset_nav();
             else {
...
//	DANGEROUS STUFF REMOVED FOR NOW
        ...
        #define BasePIDtime 3000.0f                                        // Defines the Timebase the PID controller refers to so you can keep your PIDS
