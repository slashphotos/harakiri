...
extern char * const mixerNames[];  // to displa the current mixer
...
unsigned char digit10000(uint16_t v) {return '0' + v / 10000;}
unsigned char digit1000(uint16_t v) {return '0' + v / 1000 - (v/10000) * 10;}
unsigned char digit100(uint16_t v) {return '0' + v / 100 - (v/1000) * 10;}
unsigned char digit10(uint16_t v) {return '0' + v / 10 - (v/100) * 10;}
unsigned char digit1(uint16_t v) {return '0' + v - (v/10) * 10;}
void i2c_OLED_LCDattributesOff() {CHAR_FORMAT = 0; }


void i2c_OLED_send_cmd(uint8_t command) {

  i2cWrite(OLED_address, 0x80, (uint8_t)command);
}


void i2c_OLED_send_byte(uint8_t val) {
  i2cWrite(OLED_address, 0x40, (uint8_t)val);
}
...
void i2c_LCDattributesBold() {/*CHAR_FORMAT = 0b01111111; */}
...
bool  i2c_OLED_init(void){
    ...
    ack = i2cRead(OLED_address, 0xA0, 1, &dummy);
    if (ack){
        i2c_OLED_send_cmd(0xae);    //display off
...
bool initI2cLCD(bool cli) {
  ...
  if (i2c_OLED_init())
  {
    ...
    i2c_OLED_LCDsetLine(4); i2c_OLED_LCDprintChar("YAW RIGHT & PITCH FWD");
    return true;
  } else return false;
}
